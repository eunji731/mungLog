...
export const Calendar: React.FC<CalendarProps> = ({ 
  ...
  const getMarkerColor = (type: string | number, isSelected: boolean) => {
    ...
    switch (code) {
      ...
      case 'EXPENSE': return 'bg-stone-400 dark:bg-stone-500'; 
  ...
  return (
    <div className={`select-none text-foreground ${className}`}>
      <header className="flex justify-between items-center mb-10 px-2">
        <h3 className="text-[36px] md:text-[44px] font-black tracking-tighter leading-none text-foreground">
          {monthLabel}
        </h3>
        <div className="flex gap-2 bg-surface-green p-1.5 rounded-[20px] border border-border">
          <button onClick={() => setCurrentMonth(prev => moveMonth(prev, -1))} className="w-10 h-10 flex items-center justify-center rounded-[14px] bg-background border border-border text-text-sub hover:text-main-green hover:border-main-green shadow-sm transition-all active:scale-90">
      ...
      <div className="grid grid-cols-7 text-center">
        {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map((d, i) => (
          <div key={d} className={`text-[10px] font-black tracking-[0.2em] pb-8 ${i === 0 ? 'text-red-400/70' : 'text-text-sub/65'}`}>
            {d}
          </div>
        ))}
          ...
          return (
            <div 
              ...
              className={`relative h-[72px] md:h-[84px] flex flex-col items-center justify-center rounded-2xl transition-all duration-300 cursor-pointer group
                ...
                ${isSelected ? 'bg-main-green text-white shadow-xl shadow-main-green/20 scale-105 z-10' : 'hover:bg-main-green/5'}
              `}
            >
              <span className={`text-[18px] md:text-[20px] font-black tabular-nums ${isSelected ? 'text-white' : isToday ? 'text-main-green border-b-2 border-main-green' : 'text-foreground'}`}>
                {dayNumber}
              </span>
          ...
          {legendType === 'CARE' ? (
            ...
            recordTypes.map(type => (
              <div key={type.id} className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-black text-text-sub uppercase tracking-widest">{type.codeName}</span>
              </div>
            ))
          ) : (
